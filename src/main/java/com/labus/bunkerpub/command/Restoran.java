...
@Data
public class Restoran implements Command<EditMessageText> {
    ...
    public EditMessageText execute() {
        ...
        for(int i=0;i<6;i++){
            ...
            for(int k=1;k<3;k++){
                buttons.add(new InlineKeyboardButton().setText("Стіл №"+(2*i+k)).setCallbackData("Ресторан_"+(2*i+k)));
            }
        ...
        bottomButton.add(new InlineKeyboardButton().setText("\uD83D\uDCDE").setCallbackData("tl_Ресторан"));
